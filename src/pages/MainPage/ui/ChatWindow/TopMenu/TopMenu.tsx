...
export const TopMenu = (props: PropsType) => {
    ...
    return (
        <div style={{width: '100%', height: 46, marginBottom: 2}}>
            {isVisibleConfirmChatDeleteModal && <ConfirmChatDeleteModal visible={isVisibleConfirmChatDeleteModal}
                                                                        setVisible={setVisibleConfirmChatDeleteModal}/>}
            <Flex gap={'small'} justify={'space-between'} align={'center'} style={{height: '100%'}}>
                <Flex gap={'small'} align={'center'}>
                    <Avatar src="https://storage.ws.pho.to/s2/6b3b4c3d6708259901c7ab83f3bcaa8306d63a31_m.jpeg"
                            size={"large"}/>
                    <Text strong>{selectedConversation?.title}</Text>
                </Flex>
                <Dropdown menu={{items: menuItems, onClick: menuItemsClickHandler}}>
                    <Button type={'text'} style={{color: '#000'}} size={'large'} icon={<MoreOutlined/>}/>
                </Dropdown>
